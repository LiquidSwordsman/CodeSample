...
namespace CodeSample.Generators
{
    ...
    public static class MapGenerator
    {
                ...
                (numberOfRoomCreationAttempts <= rangeOfPossibleRooms.Max()))
            {
                ...
                else
                {
                    if (firstRoom == null)
                    {
                        ...
                        newRoomCenter = firstRoom.Center();
                    }
                    else
                    {
        ...
        private static Room RoomBuilder(List<Room> generatedRooms, bool allowIntersections = false)
        {
            ...
            Tuple<Range, Range> possibleRoomDimensions = MapGenParser.GetRoomSizeRange();
            int roomWidth = RNG.Next((int)possibleRoomDimensions.Item1.Min(), 
                                     (int)possibleRoomDimensions.Item1.Max());
            int roomHeight = RNG.Next((int)possibleRoomDimensions.Item2.Min(), 
                                    (int)possibleRoomDimensions.Item2.Max());
        ...
        private static float GetRandomFloat(float minimum, float maximum)
        {
            return (float)(minimum + (RNG.NextDouble() * (maximum - minimum)));
        }
